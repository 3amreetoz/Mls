...
class course_edit_form extends moodleform
{
    ...
    function definition()
    {
        ...
        if ($CFG->downloadcoursecontentallowed) {
            ...
            if (
                (!empty($course->id) && !has_capability('moodle/course:configuredownloadcontent', $coursecontext)) ||
                (empty($course->id) &&
        ...
        if (!empty($CFG->enablecourserelativedates)) {
            ...
            $relativedatesmodegroup[] = $mform->createElement('html',
                html_writer::span(
        ...
        $mform->addElement('text', 'price', get_string('pricecourse'), 'maxlength="10"  size="10"');
        $mform->addHelpButton('price', 'pricecourse');
        $mform->setType('price', PARAM_RAW);
        // if (!empty($course->id) && !has_capability('moodle/course:changeprice', $coursecontext)) {
        //     // $mform->hardFreeze('price');
        //     $mform->setConstants('price', $course->price);
        // }
        ...
        if (
            (empty($course->id) && guess_if_creator_will_have_course_capability('moodle/course:setforcedlanguage', $categorycontext))
        ...
        if (
            (empty($course->id) && guess_if_creator_will_have_course_capability('moodle/course:renameroles', $categorycontext))
